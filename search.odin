...
draw_search_box :: proc() {
	...
	if begin("SearchBox", pad = 8, gap = 4) {
		...
		badge_width := f32(0)
		if search.filter != "" {
			badge_width = min(fx.measure_text(search.filter, 18).x + 32, bounds.size.x * 0.5)
		}
		...
		if badge_width > 0.5 {
			...
			fx.draw_text_faded(search.filter, fx.Rect{{rect.pos.x + 24, rect.pos.y}, {badge_width - 32, rect.size.y}}, 18, fx.WHITE)
		}
		...
		if len(text) == 0 && !search.focused {
			fx.draw_text_faded("Search tracks, artists, lyrics", text_rect, 24, COLOR_MUTED)
		} else {
			draw_textbox(text_rect, 24)
		}
